...
import { getGigs } from 'apiSdk/gigs';
import { GigInterface } from 'interfaces/gig';
import { getBids } from 'apiSdk/bids';
import { BidInterface } from 'interfaces/bid';

import { BidListPage as ShowCaseList } from 'pages/bids';

function DashboardPage() {
  const { session } = useSession();

  const gigStat = useSWR<PaginatedInterface<GigInterface>>(
    () => `/gigs`,
    () => getGigs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Gigs',
      result: gigStat?.data?.totalCount,
      isLoading: gigStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bids`} passHref legacyBehavior>
                  <Button
