...
import { getGigs, deleteGigById } from 'apiSdk/gigs';
import { GigInterface } from 'interfaces/gig';

type ColumnType = ColumnDef<GigInterface, unknown>;

interface GigListPageProps {
...
export function GigListPage(props: GigListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GigInterface>>(
    () => `/gigs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: GigInterface) => {
    if (hasAccess('gig', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/gigs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/gigs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('gig', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/gigs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('gig', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="gig" />
        </Flex>

        {hasAccess('gig', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/gigs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
